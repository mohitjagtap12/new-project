import '../../models/crop.dart';

class MockCrops {
  static List<Crop> initialCrops = [
    Crop(
      id: 'c1',
      name: 'Tomato',
      variety: 'Abhinav F1 Hybrid',
      area: '2 acres',
      plantingDate: '10 June 2026',
      expectedHarvest: '15 September 2026',
      status: 'Growing',
      image: 'https://images.unsplash.com/photo-1592924357228-91a4daadcfea?w=600&auto=format&fit=crop&q=80',
      notes: 'Planted with drip irrigation. Applied organic vermicompost.',
    ),
    Crop(
      id: 'c2',
      name: 'Wheat',
      variety: 'Sharbati HD-2967',
      area: '3.5 acres',
      plantingDate: '15 November 2025',
      expectedHarvest: '20 March 2026',
      status: 'Ready for Harvest',
      image: 'https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?w=600&auto=format&fit=crop&q=80',
      notes: 'Grain filling complete. Combine harvester scheduled.',
    ),
    Crop(
      id: 'c3',
      name: 'Onion',
      variety: 'Nasik Red (Fursungi)',
      area: '1.5 acres',
      plantingDate: '01 July 2026',
      expectedHarvest: '25 October 2026',
      status: 'Growing',
      image: 'https://images.unsplash.com/photo-1618512496248-a07fe83aa8cb?w=600&auto=format&fit=crop&q=80',
      notes: 'Raised beds with mulching sheet. Good canopy growth.',
    ),
    Crop(
      id: 'c4',
      name: 'Sugarcane',
      variety: 'Co-86032 (Nira)',
      area: '4 acres',
      plantingDate: '05 January 2026',
      expectedHarvest: '10 December 2026',
      status: 'Growing',
      image: 'https://images.unsplash.com/photo-1543083477-4f785aeafaa9?w=600&auto=format&fit=crop&q=80',
      notes: 'Inter-cultivation complete. Fertigation weekly.',
    ),
  ];

  static List<CropSale> initialCropSales = [
    CropSale(
      id: 'cs1',
      cropId: 'c1',
      cropName: 'Tomato',
      variety: 'Abhinav F1 Hybrid',
      quantity: '500',
      unit: 'kg',
      price: 25.0,
      priceUnit: '₹/kg',
      availableDate: '15 Sep 2026',
      location: 'Baramati, Pune',
      image: 'https://images.unsplash.com/photo-1592924357228-91a4daadcfea?w=600&auto=format&fit=crop&q=80',
      description: 'Fresh farm-harvested red ripe tomatoes. High shelf life and grade-A firmness.',
      status: 'Active',
      postedDate: '01 Sep 2026',
    ),
    CropSale(
      id: 'cs2',
      cropId: 'c2',
      cropName: 'Wheat',
      variety: 'Sharbati HD-2967',
      quantity: '12',
      unit: 'quintal',
      price: 3200.0,
      priceUnit: '₹/quintal',
      availableDate: '20 Mar 2026',
      location: 'Haveli, Pune',
      image: 'https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?w=600&auto=format&fit=crop&q=80',
      description: 'Cleaned grade-A Sharbati wheat grains, moisture tested at 11%.',
      status: 'Sold',
      postedDate: '20 Aug 2026',
    ),
    CropSale(
      id: 'cs3',
      cropId: 'c3',
      cropName: 'Onion',
      variety: 'Nasik Red (Fursungi)',
      quantity: '40',
      unit: 'quintal',
      price: 2200.0,
      priceUnit: '₹/quintal',
      availableDate: '25 Oct 2026',
      location: 'Junnar, Pune',
      image: 'https://images.unsplash.com/photo-1618512496248-a07fe83aa8cb?w=600&auto=format&fit=crop&q=80',
      description: 'Cured medium-large red onions ready for long-distance mandi dispatch.',
      status: 'Active',
      postedDate: '28 Aug 2026',
    ),
  ];
}
